...
import { CommandConsole } from '../components/family/CommandConsole';
import { CommunicationLog } from '../components/family/CommunicationLog';
import { SystemProtocols } from '../components/family/SystemProtocols';
import { NetworkTopology } from '../components/family/NetworkTopology';
import { MemberDetailPanel } from '../components/family/MemberDetailPanel';
import { BackendPanel } from '../components/family/BackendPanel';
import { FiveDimensionsPanel } from '../components/family/FiveDimensionsPanel';
import { PhilosophyFramework } from '../components/family/PhilosophyFramework';
import { TechAuditPanel } from '../components/family/TechAuditPanel';
import { KnowledgeBasePanel } from '../components/family/KnowledgeBasePanel';
import { IntegratedTerminal } from '../components/family/IntegratedTerminal';
...
import { Activity, Radio, Terminal as TerminalIcon, Network, Router, Zap, Wifi, WifiOff, Cable, Orbit, Users2, BookOpen, PanelLeftClose, PanelLeftOpen, ClipboardCheck, Database, ChevronDown, ChevronUp, Code2, Blocks } from 'lucide-react';
import { cn } from '@/components/ui/utils';
import { ImageWithFallback } from '../components/figma/ImageWithFallback';

/**
 * ChatRoomLayout - AI Family 聊天室布局
 * 
 * 职责：
 * - 家族成员头像栏（底部左侧可折叠竖向 rail）
 * - 对话消息流（占据主要区域）
 * - 命令输入控制台（底部右侧）
 * - 顶部工具栏（TOOLS 下拉菜单集成 8 个面板入口）
 * 
 * 重构说明：
 * - 从 FamilyDashboard.tsx 迁移而来
 * - 保持所有功能逻辑完全不变
 * - 为多模态工作台架构奠定基础
 */
export const ChatRoomLayout: React.FC = () => {
  ...
  // ---- 处理来自 useFamilySystem 的待处理 UI 动作 ----
  useEffect(() => {
  ...
  const handleNavigateToMember = (roleId: RoleId) => {
    panelManager.closePanel(); // 关闭任何打开的面板
               ...
               <div className="flex-1 flex flex-col items-center justify-center text-slate-600 p-8 text-center">
                 ...
                 <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
                   <button
                     ...
                     className="flex items-center gap-2 px-4 py-2 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 transition-all font-mono text-xs"
                   >
                   ...
                   <button
                     ...
                     className="flex items-center gap-2 px-4 py-2 rounded-lg bg-violet-500/10 hover:bg-violet-500/20 text-violet-400 border border-violet-500/30 transition-all font-mono text-xs"
                   >
